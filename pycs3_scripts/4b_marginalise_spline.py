...
def main(lensname, dataname, work_dir='./'):
    ...
    combkw_marg = [[f"{config.optfctkw}_ks{config.knotstep_marg[i]}_{config.mlname}_{string_ML}_{config.mlknotsteps_marg[j]}"
    ...
    for a, kn in enumerate(config.knotstep_marg):
        for b, ml in enumerate(config.mlknotsteps_marg):
            for n, noise in enumerate(config.tweakml_name_marg_spline):
                ...
                f.write('Set %s, knotstep : %2.2f, %s : %s \n' % (name, kn, string_ML, ml))
    ...
    print(medians_list.shape)
    print(errors_up_list.shape)
    binslist = []
    ...
    combined, surviving_groups = copy.deepcopy(
    ...
    if config.display:
        pycs3.tdcomb.plot.delayplot(group_list + [combined], rplot=radius, refgroup=combined, text=text, hidedetails=True,
                                  ...
                                  tick_step_auto=True)
    ############# TMP blinding
    #mmult = lambda x: x * 1.0928
    #for gg in group_list + [combined]:
    #    gg.medians =  list(map(mmult, gg.medians))
    #    gg.errors_up = list(map(mmult, gg.errors_up))
    #    gg.errors_down = list(map(mmult, gg.errors_down))
    ...
    pkl.dump(combined,
             open(marginalisation_dir + config.name_marg_spline + "_sigma_%2.2f" % config.sigmathresh + '_combined.pkl',
                  'wb'))
    pkl.dump(surviving_groups,
         open(marginalisation_dir + config.name_marg_spline + "_sigma_%2.2f" % config.sigmathresh + '_groups_used_in_combined.pkl',
              'wb'))
